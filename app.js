...
fs.readdir("./cmd/", (err, files) => {
    ...
    console.log(`Toplam ${files.length} Komut yükleniyor.`);
    files.forEach(f => {
      let props = require(`./cmd/${f}`);
      console.log(`Yüklenen Komut: ${props.help.name}. Başarılı`);
...
client.on("message", message => {
    ...
    if (client.maintenance.has(message.guild.id) && message.author.id !== settings.OwnerID) return message.channel.send("Üzgünüm, ama şu an bakımdayım.");
    ...
    if (cmd) {
        if (perms < cmd.conf.permLevel) return message.channel.send(`Üzgünüm, ama bu komut izin seviyesi gerektirir ${cmd.conf.permLevel} yada üstü.`);
...
client.on("ready", () => {
    ...
    if (client.guilds.size === 1) totalServers = "Sunucu";
    if (client.guilds.size > 1) totalServers = "Sunucu";
    client.user.setActivity(`>>Komutlar için !yardım Yazın!`, {type: `WATCHING`});
});
